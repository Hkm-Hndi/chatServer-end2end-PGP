...
namespace Server
{
    ...
    public class ChatService : ISendChatService 
    {
        ...
        void ISendChatService.SendMessage(byte[] msg,string sender, string receiver)
        {
            if (names.ContainsKey(receiver))
            {
                File.WriteAllText(System.Environment.CurrentDirectory + "\\" + "EncryptedMsg.xml", string.Empty);
                File.WriteAllBytes(System.Environment.CurrentDirectory + "\\" +"EncryptedMsg.xml", msg);
